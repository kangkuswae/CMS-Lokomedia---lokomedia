...
elseif ($_GET['module']=='detailberita'){
  ...
  $komentar = mysqli_query($conn,"select count(komentar.id_komentar) as jml from komentar WHERE id_berita='$idnya' AND aktif='Y'");
  $k = mysqli_fetch_array($komentar); 
  echo "<span class=judul><b>$k[jml]</b> Komentar : </span><br /><hr color=#e0cb91 noshade=noshade />";

  // Komentar berita
  $sql = mysqli_query($conn,"SELECT * FROM komentar WHERE id_berita='$idnya' AND aktif='Y'");
  ...
  if ($jml > 0){
    while ($s = mysqli_fetch_array($sql)){
      ...
      // Apabila ada link website diisi, tampilkan dalam bentuk link   
 	    if ($s['url']!=''){
        echo "<span class=komentar><a href='$s[url]' target='_blank'>$s[nama_komentar]</a></span><br />";  
	    }
	    else{
...
elseif ($_GET['module']=='hasilpoling'){
   ...
   if(isset($_POST['pilihan'])){
     ...
     $pilihan = isset($_POST['pilihan']);
    $u=mysqli_query($conn,"UPDATE poling SET rating=rating+1 WHERE id_poling='$pilihan'");

    echo "<p align=center>Terimakasih atas partisipasi Anda mengikuti poling kami</p><br /><br />";
   }
  
    echo " <p>     Hasil poling saat ini: </p>";
...
elseif ($_GET['module']=='semuaberita'){
  ...
  $sql=mysqli_query($conn,"select count(komentar.id_komentar) as jml, judul, judul_seo, jam, 
                       berita.id_berita, hari, tanggal, gambar, isi_berita    
                       from berita left join komentar 
                       on berita.id_berita=komentar.id_berita
                       and aktif='Y' 
